...
// License: GNU General Public License v3. See license.txt
        ...
        $(document).ready(function () {
            frappe.call({
                ...
                callback: function (r) {
                    frappe.db.get_value('Stock Entry', { name: stockEntryName }, 'production_order', (r) => {
						frappe.db.get_value('Production Order', { name: r.production_order }, 'boxes_per_case', (p) => {
							boxesPerCase = p.boxes_per_case;
							ProcessCaseStatus();
						});
						
						frappe.db.get_value('Production Order', { name: r.production_order }, 'box_item', (p) => {
							boxItem = p.box_item;
						});

                        $('#ProductionOrderLink').text(r.production_order).attr('href', 'https://erp.lohxa.com/desk#Form/Production%20Order/' + r.production_order);
                    });
