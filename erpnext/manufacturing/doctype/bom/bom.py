...
def get_bom_items_as_dict(bom, company, qty=1, fetch_exploded=1, fetch_scrap_items=0, include_non_stock_items=False, fetch_qty_in_stock_uom=True):
	...
	if cint(fetch_exploded):
		...
		query = query.format(table="BOM Item", where_conditions="", is_stock_item=is_stock_item,
			...
			select_columns = ", bom_item.uom, bom_item.conversion_factor, bom_item.is_additional_output, bom_item.source_warehouse, bom_item.idx, bom_item.operation, bom_item.include_item_in_manufacturing",
			groupby_columns = """, bom_item.operation""")
		items = frappe.db.sql(query, { "qty": qty, "bom": bom, "company": company }, as_dict=True)
